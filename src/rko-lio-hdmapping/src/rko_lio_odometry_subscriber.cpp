...
int main(int argc, char **argv)
{
    ...
    while (bag.has_next())
    {
        ...
        // Process per-frame point clouds from /rko-lio/frame topic
        // This topic publishes per-frame registered points (always enabled, not dependent on visualize)
        if (msg->topic_name == "/rko_lio/frame") {
            RCLCPP_INFO(rclcpp::get_logger("rko-lio-frame"), "Received message on topic: /rko_lio/frame");
            ...
            if (!cloud_msg || cloud_msg->data.empty()) {
                ...
                RCLCPP_DEBUG(rclcpp::get_logger("rko-lio-frame"), "Empty PointCloud2 message on /rko-lio/frame");
                continue;
            }
    
            size_t num_points = cloud_msg->width * cloud_msg->height;
    
            RCLCPP_INFO(rclcpp::get_logger("rko-lio-Frame"), "Processing %zu points from /rko-lio/frame", num_points);
            ...
            RCLCPP_INFO(rclcpp::get_logger("rko-lio-frame"), "Added %zu points (total: %zu)", num_points, points_global.size());
        }
        
        if (msg->topic_name == "/rko_lio/odom") {
            RCLCPP_INFO(rclcpp::get_logger("rko-lio-odometry"), "Received message on topic: /rko_lio/odom");
            ...
            if (!odom_msg) {
                RCLCPP_ERROR(rclcpp::get_logger("rko-lio-odometry"), "Odometry message deserialization error!");
                return 1;
            }
            ...
            RCLCPP_INFO(rclcpp::get_logger("rko-lioOdometry"), "Added position to trajectory: x=%.3f, y=%.3f, z=%.3f", x, y, z);
        }
    }
